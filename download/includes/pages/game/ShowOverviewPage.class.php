	...
	function show()
	{
		...
		{		
			...
			if (!empty($CPLANET['b_building']) && $CPLANET['b_building'] > TIMESTAMP) {
				...
				$BuildPlanet		= '
				<a class="constructionIcon js_hideTipOnMobile tooltip" href="game.php?page=overview" title="'.$LNG['tech'][$Queue[0][0]].'">
				<span class="icon12px icon_wrench"></span>
				</a>'; /* $LNG['tech'][$Queue[0][0]]." (".$Queue[0][1].")<br><span style=\'color:#7F7F7F;'>(".pretty_time($Queue[0][3] - TIMESTAMP).")</span>" */
			} else {
				$BuildPlanet     = '';
			}
