...
<input type="hidden" value="1" name="send">
	...
	<tr>
			<th colspan="{count($avalibleRanks) + 2}">{$LNG.al_configura_ranks} <span style="float:right;"><button class="button small blue" id="create_new_alliance_rank">+</button></span></th>
	</tr>
	<tr>
		<td>{$LNG.al_dlte}</td>
		<td>{$LNG.al_rank_name}</td>
		{foreach $avalibleRanks as $rankName}
		<td><img class="tooltip" data-tooltip-content="{$LNG.al_rank_desc[$rankName]}" src="styles/resource/images/alliance/{$rankName}.png" alt="" width="16" height="16"></td>
		{/foreach}
	</tr>
	{foreach $rankList as $rankID => $rankRow}
	<tr>
		<td><a class="tooltip" data-tooltip-content="{$LNG.al_dlte}" href="game.php?page=alliance&amp;mode=admin&amp;action=permissionsSend&amp;deleteRank={$rankID}"><img src="styles/resource/images/alliance/CLOSE.png" alt="" width="16" height="16"></a></td>
	...
	{/foreach}
	<tr>
		<td colspan="{$countRank + 2}"><input class="button small green" type="submit" value="{$LNG.al_save}"></td>
	</tr>
	<tr>
	...
	<tr>
		...
		{/foreach}
	<tr>
			<th style="text-align:center" colspan="{$countRank + 2}"><a class="button black small" href="game.php?page=alliance&amp;mode=admin">{$LNG.al_back}</a></th>
	</tr>
	</table>
</form>
...
<div id="new_alliance_rank" title="{$LNG.al_create_new_rank}" style="display:none;">
	...
	{/foreach}
	<tr>
				<td colspan="{$countRank + 2}"><input class="button medium blue" type="submit" value="{$LNG.al_create}"></td>
	</tr>
	</table>
</form>
</div>
